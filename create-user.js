...
export class DatabaseStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    try {
      // Use exact database column names with Drizzle's syntax
      const [user] = await db.insert(users).values({
        username: insertUser.username,
        email: insertUser.email,
        display_name: insertUser.displayName, // Direct database column name
        password: insertUser.password,
        avatar_url: insertUser.avatarUrl || null // Direct database column name
      } as any).returning(); // Use "as any" to bypass TypeScript checks
